...
VIDEO_UPDATE( cps1 )
{
	...
	if (state->cps_version == 2)
	{
	cps2_set_sprite_priorities(screen->machine);                                              //ghetto fix
	memcpy(state->cps2_buffered_obj, cps2_objbase(screen->machine), state->cps2_obj_size);
	}
	if (state->cps_version == 1)
	{
	cps1_get_video_base(screen->machine);                                             //ghetto fix
