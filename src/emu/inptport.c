...
static void frame_update_digital_joysticks(running_machine *machine)
{
	...
	for (player = 0; player < MAX_PLAYERS; player++)
		for (joyindex = 0; joyindex < DIGITAL_JOYSTICKS_PER_PLAYER; joyindex++)
		{
			...
			if (joystick->inuse)
			{
				joystick->previous_from_seq = joystick->current_from_seq;
				...
				/* by accepting the most recent one */
				joystick->current_from_seq = joystick->current;

				if ((joystick->current & (JOYDIR_UP_BIT | JOYDIR_DOWN_BIT)) == (JOYDIR_UP_BIT | JOYDIR_DOWN_BIT))
				{
					if ((joystick->previous_from_seq & (JOYDIR_UP_BIT | JOYDIR_DOWN_BIT)) == (JOYDIR_UP_BIT | JOYDIR_DOWN_BIT))
						joystick->current ^= ~(joystick->previous) & (JOYDIR_UP_BIT | JOYDIR_DOWN_BIT);
					else if (joystick->previous & (JOYDIR_UP_BIT | JOYDIR_DOWN_BIT))
						joystick->current ^= joystick->previous & (JOYDIR_UP_BIT | JOYDIR_DOWN_BIT);
					else
						/* if up and down are pressed together, favor up */
						joystick->current &= ~(JOYDIR_DOWN_BIT);
				}
				if ((joystick->current & (JOYDIR_LEFT_BIT | JOYDIR_RIGHT_BIT)) == (JOYDIR_LEFT_BIT | JOYDIR_RIGHT_BIT))
				{
					if ((joystick->previous_from_seq & (JOYDIR_LEFT_BIT | JOYDIR_RIGHT_BIT)) == (JOYDIR_LEFT_BIT | JOYDIR_RIGHT_BIT))
						joystick->current ^= ~(joystick->previous) & (JOYDIR_LEFT_BIT | JOYDIR_RIGHT_BIT);
					else if (joystick->previous & (JOYDIR_LEFT_BIT | JOYDIR_RIGHT_BIT))
						joystick->current ^= joystick->previous & (JOYDIR_LEFT_BIT | JOYDIR_RIGHT_BIT);
					else
						/* if left and right are pressed together, lock both */
